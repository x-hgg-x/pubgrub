...
impl DependencyProvider for Provider {
    ...
    fn prioritize(&mut self, package: Package, _: &Self::VS) -> Self::Priority {
    ...
    fn get_dependencies(
        ...
        let mut add_dep = |arena: &mut FnvIndexSet<_>, (d, v)| {
            ...
            map.extend([(p, Range::singleton(v))]);
        };

        let wrapper = &self.package_to_name(package).unwrap().0;
        let inner = wrapper.inner(version).map(|(&p, v)| (p, v));
        ...
        match wrapper.dependency(version, info_fn) {
            Ok(None) => (),
            Ok(Some((d, v))) => {
...
fn main() {
    let (root_pkg, root_pkg_version) = CustomPackage::from_pkg("root", 1);
    ...
    match resolve(&mut provider, &root_pkg, root_pkg_version) {
        Ok(sol) => {
            for (p, v) in sol {
                println!("{p}: {v}");
            }
