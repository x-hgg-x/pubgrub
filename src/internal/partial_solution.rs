...
impl<M: Eq + Clone + Debug + Display> Display for PackageAssignments<M> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
...
#[derive(Clone, Debug)]
struct DatedDerivation<M: Eq + Clone + Debug + Display> {
    global_index: u32,
...
impl<M: Eq + Clone + Debug + Display> Display for DatedDerivation<M> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
...
#[derive(Clone, Debug)]
pub(crate) enum SatisfierSearch<M: Eq + Clone + Debug + Display> {
